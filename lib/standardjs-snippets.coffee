...
module.exports =
  ...
  activate: (state) ->
    atom.commands.add 'atom-text-editor',
      'standard-javascript:end-line-comma': => @endLine(',', false)
    atom.commands.add 'atom-text-editor',
      'standard-javascript:end-new-line': => @endLine('', true)
    atom.commands.add 'atom-text-editor',
      'standard-javascript:wrap-block': => @wrapBlock()
  ...
  endLine: (terminator, insertNewLine) ->
    ...
    editor.getCursors().forEach((cursor) ->
      ...
      if !terminator
        ...
        terminator = if objectLiteralLine.test(line) then ','

      editor.insertText(terminator) if terminator and !continuationLine.test(line) and !emptyLine.test(line)
